...
bundles. In the current repo state, the corpus freeze is complete while the
final downstream result bundle is still being regenerated from those frozen
artifacts.
...
next milestone is to finish the release bundle over the already-frozen corpora,
populate the short paper with the saved tables and case studies, and rerun the
hosted retry baselines from an environment-provided API key rather than any
