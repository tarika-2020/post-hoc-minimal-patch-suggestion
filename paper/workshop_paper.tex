...
cases and $24$ controlled synthetic failures across retail and airline. These
results motivate replayable execution intervention as a practical debugging
protocol for LLM agents: more causal than prompt-only retry, more interpretable
than unconstrained regeneration, and naturally aligned with benchmark
evaluation.
\end{abstract}
...
thirty-two-case smoke setting to a frozen $80$-case natural corpus, while the
final artifact-backed workshop tables are still being regenerated from those
saved failures.
...
state, the corpus freeze is complete while the final downstream result bundle
is still being regenerated from those frozen artifacts.
...
\begin{itemize}
  ...
  \item the workshop artifact path now supports streaming JSONL corpora,
  per-domain shards, and resumable result writes for long reruns.
\end{itemize}

What still separates the workshop version from a full conference paper:
\begin{itemize}
  \item completed final workshop result tables and figures from the frozen
  corpora,
...
next milestone is to finish the clean canonical reruns over the already-frozen
corpora, populate the short paper with the saved tables and case studies, and
refresh the hosted retry baselines from an environment-provided API key rather
than any tracked project document.
